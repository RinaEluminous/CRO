...
function goldIraLPMinimalPage() {
  ...
  if (rowNumber6 && rowNumber7) {
    ...
    if (document.querySelectorAll('#custFreeGoldIraKit').length == 0) {
  ...
  const intSwiperInterval = setInterval(() => {
    ...
    if (typeof Swiper !== 'undefined') {
      var swiper = new Swiper(".custReviewsMain .custReviewslider", {
        ...
        mousewheel: {
          forceToAxis: true, // Constrain scrolling to one axis
        },
        keyboard: {
          enabled: true, // Enable keyboard control
        },
        speed: 800, // Adjust speed of slide transition
        breakpoints: {
          991: {
            slidesPerView: 3,
            spaceBetween: 24,
        ...
        on: {
          slideChangeTransitionEnd: function() {
            // Ensure autoplay restarts smoothly after each slide
