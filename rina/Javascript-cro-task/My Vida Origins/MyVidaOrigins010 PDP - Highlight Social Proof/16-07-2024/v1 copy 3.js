...
'</div>';
    ...
    if (document.querySelectorAll('.custFlickitySliderCSS').length ==0) {
      ...
      overlay.addEventListener('click', hidePopup);
        ...
        flickityMain.on('change', function(index) {
            console.log('flickityMain >>>>>>>>');
              var selectedCell = mainGallery.querySelectorAll('.gallery-cell')[index];
              ...
              var nonSelectedCells = mainGallery.querySelectorAll('.gallery-cell:not(.is-selected)');
              nonSelectedCells.forEach(function(cell) {
          ...
          flickityThumb.on('change', function(index) {
            console.log('flickityThumb >>>>>>>>');
              var selectedCell = thumbnailGallery.querySelectorAll('.gallery-cell')[index];
              var selectedVideo = selectedCell.querySelector('video');
              if (selectedVideo) {
                  selectedVideo.play();
              }
              ...
              // Stop video in previous cells
              var nonSelectedCells = thumbnailGallery.querySelectorAll('.gallery-cell:not(.is-selected)');
              nonSelectedCells.forEach(function(cell) {
                  var video = cell.querySelector('video');
                  if (video) {
                      video.pause();
...
// Function to check if Flickity is ready
function checkFlickityReady(callback) {
    var flkty = $mainGallery.data('flickity');
    if (flkty && flkty.cells.length > 0) {
        callback();
    } else {
        setTimeout(function() {
            checkFlickityReady(callback);
        }, 100); // Adjust the timeout interval as needed
    }
}

// Call checkFlickityReady to wait until Flickity is ready
checkFlickityReady(function() {
    console.log('Flickity is initialized and ready on .main-gallery');

    // Now you can add your event listener for 'change.flickity'
