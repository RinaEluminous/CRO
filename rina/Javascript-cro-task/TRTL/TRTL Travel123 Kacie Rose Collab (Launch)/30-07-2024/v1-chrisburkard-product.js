...
function updateBundleNavImgNew() {
    ...
    deskKacieRosePillowMenu.forEach(function (element) {
        ...
        var DCBElementsLink = element.querySelectorAll('[href*="/products/chris-burkard-trtl-travel-pillow-cool"]');
        ...
        if (headingElement && headingElement.textContent.includes('CHRIS BURKARD X TRTL PILLOW')) {
    ...
    mobileNavbarLinksElement.forEach(function (listItem) {
        //update all link of double bundle with chris burkard product link
        var DCBElementsLink = listItem.querySelectorAll('[href*="/products/chris-burkard-trtl-travel-pillow-cool"]');
        DCBElementsLink.forEach(function (element) {
            element.href = '/products/kacie-rose-x-trtl-travel-pillow-cool';
        });
        ...
        if (paragraph) {
            ...
            //             node.textContent = node.textContent.replace('CHRIS BURKARD X TRTL PILLOW', '');
            ...
            if (headingElementBCB) {
              ...
              headingElementBCB.childNodes.forEach(node => {
                ...
                if (!replaced && node.nodeType === Node.TEXT_NODE && node.textContent.includes('CHRIS BURKARD')) {

                   

                  node.textContent = node.textContent.replace('CHRIS BURKARD', 'KACIE ROSE');
                

                  var crossSpan = headingElementBCB.querySelector('span.custCrossSpan');
                  if (crossSpan) {
                      crossSpan.textContent = ' X '; 
                      node.textContent = node.textContent.trim(); 
                      headingElementBCB.insertBefore(crossSpan, node.nextSibling);
                  }
              ...
              var anchorElement = headingElementBCB.querySelector('a.stretched-link');
              if (anchorElement) {
                anchorElement.setAttribute('aria-label', 'Kacie Rose');
              }
            
              if (replaced) {
                var parentDiv = headingElementBCB.closest('.col-12.mt-2');
                if (parentDiv) {
                  parentDiv.classList.remove('custMobileBurkardLink');
                  parentDiv.classList.add('custMobileKacieRoseLinkNew');
                }
