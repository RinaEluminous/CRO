...
//check offer product exist if TRTL Pillow not exist
function offerProductAvailable(){
      ...
      if (!isTravelPillowExists) {
        console.log('TRTL PILLOW ORIGINAL not available');
        var trtlPillowelements = document.querySelectorAll('#cart-offcanvas-content .col-12 .col-8 a.text-decoration-none.text-uppercase');
        var removeButtons = [];
        trtlPillowelements.forEach(function (element) {
            ...
            if (passportParentElement) {
                ...
                if (passportOfferDiv) {
                    if (passportOfferDiv.textContent.includes("Free gift") || passportOfferDiv.textContent.includes("10% of on cart upsell")) {
                        var removeButton = passportParentElement.querySelector('[data-mp-cart-item-remove]');
                        if (removeButton) {
        ...
        // Click all remove buttons at once
...
function operationClickUK() {
    ...
    cartDltButtons.forEach(function (button) {
        button.addEventListener('click', function () {
            ...
            var parentElement = button.closest('.g-3.align-items-center');
            ...
            if (specialOfferProductCount == 1) {
                var removeBtn = document.querySelector('.custRemovebtnPassPort');
                if (removeBtn) {
                    setTimeout(function () {
