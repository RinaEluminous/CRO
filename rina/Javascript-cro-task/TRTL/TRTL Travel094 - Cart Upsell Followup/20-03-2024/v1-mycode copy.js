...
(function () {
   ...
   // Check if TRTL TRAVEL PILLOW exist
    function checkTrtlTravelPilowExist(){
    ...
    function addPassportProduct(){
        ...
        //check free passport already added or not
        var trtlPillowelements = document.querySelectorAll('#cart-offcanvas-content .col-12 .col-8 a.text-decoration-none.text-uppercase');
        trtlPillowelements.forEach(function(element) {
            if (element.textContent.includes("TRTL PASSPORT COVER")) {
                console.log('TRTL PASSPORT COVER >>');
                var passportParentElement = element.closest('.cart-items-selector');
                if (passportParentElement) {
                    var passportOfferDiv = passportParentElement.querySelector('.font-heading');
                    if (passportOfferDiv) { 
                        console.log('<<<<<<<<<<>>',passportOfferDiv.textContent);
    ...
    function updateTextOnOfferProduct(){
        ...
        trtlPillowelements.forEach(function(element) {
            ...
            if (parentElement) {
                ...
                if (offerDiv) { 
                    if (offerDiv.textContent.includes("10% of on cart upsell")) { 
                        console.log('testdddddddddddddd>>>>>>>');
            ...
            //passport covert product conditions
            if (element.textContent.includes("TRTL PASSPORT COVER")) {
                console.log('TRTL PASSPORT COVER >>');
                var passportParentElement = element.closest('.cart-items-selector');
                if (passportParentElement) {
                    ...
                    if (passportOfferDiv) { 
                        console.log('<<<<<<<<<<>>',passportOfferDiv.textContent);
                        if (passportOfferDiv.textContent.includes("Free gift")) {
                            console.log('hide passport cover box from the special product');
    ...
    button.addEventListener('click', function() {
     ...
     //Mini cart delete operations
        ...
        cartDltButtons.forEach(function(button) {
            button.addEventListener('click', function() {
                console.log('delete cart operations:');
                var trtlPillowelements = document.querySelectorAll('#cart-offcanvas-content .col-12 .col-8 a.text-decoration-none.text-uppercase');
                trtlPillowelements.forEach(function(element) {
                var parentElement = element.closest('.cart-items-selector');
                    if (parentElement) {
                        ...
                        if (offerDiv) { 
                            if (!offerDiv.textContent.includes("10% of on cart upsell")) { 
                                console.log('10% of on cart upsell not exist in the all cart');
                                if (element.textContent.includes("TRTL PASSPORT COVER")) {
                                    var passportParentElement = element.closest('.cart-items-selector');
                                        if (passportParentElement) {
                                            var passportOfferDiv = passportParentElement.querySelector('.font-heading');
                                            if (passportOfferDiv) { 
                                               if (passportOfferDiv.textContent.includes("Free gift")) {
                                                removeButton = passportParentElement.querySelector('button[data-mp-cart-item-remove=""]');
                                                if (removeButton) {
                                                    console.log('if passport exist with free gift then remove it');
                                                    removeButton.click();
                                                    document.querySelector('.custPassportCoverBox')?.classList.remove('custHideContent');
                                                }
