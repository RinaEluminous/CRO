...
function addDefault2Pack() {
    ...
    if (sessionStorage.getItem('callCheckAndInitialize') === 'true') {
     var closeButton = document.querySelector(".justify-content-end .xposition-relative img");
        if (closeButton) {
            closeButton.click();
         }
        sessionStorage.removeItem("isTRTLEadsProductAddedInCart");
        sessionStorage.removeItem('callCheckAndInitialize');
    ...
    if (urlPath == '/products/trtl-travel-pillow-ads3') {
        ...
        if (variantBlocksElement) {
            ...
            var custSelectOfferHtml = '<div class="custSelectBuyOfferWrapper">' +
                ...
                '                    <div class="cancelPrice text-oblique">$119.98</div>' +
                '                    <div class="finalPrice">$95.98</div>' +
                '                </div>' +
                '            </div>' +
                '        </div>' +
    ...
    if (promises.length > 0) {
        Promise.all(promises)
            .then(function () {
                ...
                setTimeout(function () {
                    sessionStorage.setItem('callCheckAndInitialize', 'true');
                    location.reload();
                }, 750);
            });
