window.onload = function () {
    ...
    function addCustomUpSellProduct() {
        var custUpSellProductHtml = '<div class="custCartFollowupWraper">'+
                                    ...
                                    '                    <div class="custSwitchBlocksSection">'+
                                    '                        <div class="custSwatchWrapper">'+
                                    '                            <div class="custSwatchColorPattern new-york custSwatchActive" data-color="New-York" title="New York">'+
                                    '                            </div>'+
                                    ...
                                    '                                    <div class="col-auto"><label class="btn btn-outline-dark px-4 custSizeChartActive" data-size="Small" for="template--15670323839169__main-2-0">S</label></div>'+
                                    '                                    <div class="col-auto"><label class="btn btn-outline-dark px-4" data-size="Medium" for="template--15670323839169__main-2-1">M</label></div>'+
                                    '                                    <div class="col-auto"><label class="btn btn-outline-dark px-4" data-size="Large" for="template--15670323839169__main-2-2">L</label></div>'+
                                    '                                </div>'+
                                    '                            </div>'+
                                    '                        </div>'+
                                    '                    </div>'+
   ...
   if (element.textContent.includes("TRTL TRAVEL PILLOW")) {
    ...
    document.addEventListener('click', function(event) {
        if (event.target.classList.contains('addBtn') && event.target.closest('.custCartFollowupWraper')) {
            console.log('addBtn clicked >>>>>>>>>>>>>>>>>>>>>>>>>>>');
            var siblings = event.target.parentElement.querySelectorAll('.custProducts-colour-variant-swatches');
    ...
    document.addEventListener('click', function(event) {
        ...
        if (event.target.matches('.custCartFollowupWraper a.custCloseCrossBlocks')) {
          var parent = event.target.parentElement;
            if (parent) {
              parent.classList.remove('custProductsOpen');
            }
        }
    });
    ...
    document.addEventListener('click', function(event) {
        ...
        if (event.target.classList.contains('custSwatchColorPattern') && event.target.closest('.custCartFollowupWraper')) {
            document.querySelectorAll('.custSwatchColorPattern').forEach(function(element) {
                element.classList.remove('custSwatchActive');
            });
            event.target.classList.add('custSwatchActive');
            var strSelectedColor = event.target.dataset.color;
            var arrayColorValue = {
                Grey: 42424477614273,
    ...
    document.addEventListener('click', function(event) {
        ...
        if (event.target.matches('#cart-offcanvas-content button.btn')) {
            setTimeout(function() {
              var targetElement = document.querySelector('#cart-offcanvas-content .col-12 .col-8 a.text-decoration-none.text-uppercase');
                if (targetElement && targetElement.textContent.includes('TRTL TRAVEL PILLOW')) {
                    addCustGetExtramsgFollowup();
                    addCustomUpSellProduct();
                    modifyExistingCartContent();
                }
    ...
    document.addEventListener('click', function(event) {
        ...
        if (event.target.matches('button.add-to-cart-button[name=add], button.sticky-add-to-cart[name=add]')) {
          setTimeout(function() {
                 var targetElement = document.querySelector('#cart-offcanvas-content .col-12 .col-8 a.text-decoration-none.text-uppercase');
                if (targetElement && targetElement.textContent.includes('TRTL TRAVEL PILLOW')) {
                    addCustGetExtramsgFollowup();
