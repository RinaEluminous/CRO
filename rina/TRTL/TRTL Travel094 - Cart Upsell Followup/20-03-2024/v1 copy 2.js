document.addEventListener('DOMContentLoaded', function() {
    ...
    console.log('TRTL Travel094 - Cart Upsell Followup >>>');
        ...
        function addCustomUpSellProduct() {
            var custUpSellProductHtml = '<div class="custCartFollowupWraper">'+
                                        ...
                                        '                <p><span>+ FREE PASSPORT COVER <em>($12.00 VALUE)</em></span> INCLUDED WHEN YOU ADD A DEAL TO CART</p>'+
                                        '            </div>'+
                                        '        </div>'+
                                        '    </div>'+
                                        '</div>';
        ...
        var shippingUnlockAlert = document.querySelector('.shipping-unlock-alert.bm-t-23-minicart-priorityText');
        if (shippingUnlockAlert !== null) {
           
            var col12Elements = Array.from(shippingUnlockAlert.nextElementSibling.parentNode.querySelectorAll('.col-12:not(:first-child):lt(4)'));
      
            if (col12Elements.length > 0) {
                var custMiniCartFeaturesDiv = document.createElement('div');
                custMiniCartFeaturesDiv.className = 'custMiniCartFeatures';
                col12Elements.forEach(function (element) {
                    custMiniCartFeaturesDiv.appendChild(element);
                });
                shippingUnlockAlert.parentNode.insertBefore(custMiniCartFeaturesDiv, shippingUnlockAlert.nextSibling);
            }
        }
        ...
        var checkoutLink = document.querySelector('#cart-offcanvas-content a[href="/checkout"]');
        if (checkoutLink !== null) {
            var newTextOfCheckout = checkoutLink.textContent.trim() + ' - ' + document.querySelector('.col-6.mt-3.text-end').textContent.trim();
            checkoutLink.textContent = newTextOfCheckout;
        }
        }
   ...
   custCompressionSocks.addEventListener('click', function(event) {
    ...
    if (target.matches('.custSwatchColorPattern') || target.matches('.custSizeChart .custRow label.btn')) {
        ...
        // if (target.matches('.custSwatchColorPattern')) {
        //     console.log('Your event handling logic for .custSwatchColorPattern elements here');
          
        // }
        // For .custSizeChart .custRow label.btn elements or their descendants
        // if (target.matches('.custSizeChart .custRow label.btn')) {
        //     console.log('Your event handling logic for .custSizeChart .custRow label.btn elements here');
        // }
        ...
        if (target.classList.contains('custSwatchColorPattern')) {
            ...
            var allSizeButtons = document.querySelectorAll('#custCompressionSocks .cusColtSize div.btn');
