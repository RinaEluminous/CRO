var waitForjQuery = setInterval(function() {
	if (typeof jQuery != 'undefined') {
        addNewNavDualDropdownsV2();
        
        function addNewNavDualDropdownsV2() {
            ...
            if( jQuery('.navbar-expand ul li a:contains("Reviews")').length > 0){
                jQuery('.navbar-expand ul li a:contains("Reviews")').parent().nextAll().hide();
            }
           ...
           if( jQuery('.offcanvas-body .w-100  .g-4 .col-12 a:contains("Pillows")').length > 0 && jQuery('.offcanvas-body .w-100  .g-4 .col-12 a:contains("Accessories")').length == 0){
                ...
                var strAccessoriesMobileHtml = '<a class="nav-link p-0 fs-3 font-ibm-mono offcanvas-dropdown-toggle position-relative collapsed" data-bs-toggle="collapse" href="#nav-offcanvas-link-1-sublinks" role="button" aria-expanded="false" aria-controls="nav-offcanvas-link-1-sublinks">'+
                '    Accessories'+
                '</a>'+
                '<!-- prependTo : .section-header nav.navbar ul.navbar-nav li.nav-item.has-megamenu .megamenu .tile-line-item-menu -->'+
                ...
                '                            </picture>'+
                '                        </div>'+
                '                        <a href="/products/double-comfort-bundle-free-gift" class="stretched-link" aria-label="SLEEP MASK"></a>'+
                '                    </div>'+
                ...
                '                                Total blackout for deep sleep'+
                '                            </p>'+
                '                            <a href="https://trtltravel.com/products/trtl-flask" class="stretched-link" aria-label="SLEEP MASK"></a>'+
                '                        </div>'+
                '                    </div>'+
                '                </div>'+
                '            </div>'+
                ...
                '                            <picture class=" d-block">'+
                '                                <img loading="lazy" class="image-fade img-fluid lazyload loaded" alt="SLEEP MASK" src="https://trtltravel.com/cdn/shop/products/TPHlighter_764cefa0-ec1a-4a8a-baf2-2005925a9fb9_620x.jpg?v=1675341953">'+
                '                            </picture>'+
                '                        </div>'+
                '                        <a href="https://trtltravel.com/products/trtl-passport-cover" class="stretched-link" aria-label="PASSPORT COVER"></a>'+
                '                    </div>'+
            ...
            var joinCommunityHtml='<li class="nav-item custJoinCommunity">'+
            '    <a href="https://trtltravel.com/pages/get-lost" class="nav-link ">Join the Community</a>'+
            '</li>';

            var joinCommunityMobileHtml='<div class="col-12 custJoinMobileCommunity">'+
            '    <a class="nav-link p-0 fs-3 font-ibm-mono" href="https://trtltravel.com/pages/get-lost">Join the Community</a>'+
            '</div>';
            
            
            if(jQuery('.navbar-expand ul li a:contains("Reviews")').length > 0 && jQuery('.custJoinCommunity').length   == 0 ) {
                jQuery('.navbar-expand ul li a:contains("Reviews")').parent().after(joinCommunityHtml);
            }

            //Mobile view
            if(jQuery('.offcanvas-body .w-100  .g-4 a:contains("Reviews")').length > 0 && jQuery('.custJoinMobileCommunity').length == 0 ) {
                jQuery('.offcanvas-body .w-100  .g-4 a:contains("Reviews")').parent().after(joinCommunityMobileHtml);
            }
            
            //after reviews hide remaining div  
             if( jQuery('.offcanvas-body .w-100  .g-4 a:contains("Join the Community")').length > 0){
                var reviewsElement = jQuery('.offcanvas-body .w-100 .g-4 a:contains("Join the Community")').parent();
                var nextElements = reviewsElement.nextAll().slice(0, 3); 
                nextElements.hide();
            }
         }
