var waitForJquery = setInterval(function() {  
    if (typeof jQuery != 'undefined') {
        ...
        setTimeout(function(){
            if(jQuery('.customStickyHeaderBanner').length > 0){    
                var intHeight = jQuery('.customStickyHeaderBanner').height();
                jQuery('#fixed-header').css('top', intHeight + 'px');
                jQuery('main#MainContent').attr('style', 'padding-top: '+ intHeight +'px !important');
            }
        }, 500);

        var timerInterval = setInterval(function(){
            if(jQuery().countdown){
                var startdate = "2023/10/23 00:00:00",
        ...
        jQuery('.customStickyHeaderBanner .btn-close').on('click', function(){
            ...
            jQuery('main#MainContent').attr('style', 'padding-top: 0px !important');
        });
