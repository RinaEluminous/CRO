	    		...
	    		if (jQuery('#js-product-list .category-product').length > 0) {
	    			jQuery('#js-product-list .category-product').each(function(index) {
				        ...
				        if (index == 1 && jQuery('.custGreatProductPlpSearch').length == 0) {
				            jQuery('<div class="custGreatProductPlpSearch column half medium-one-third large-one-fourth category-product ng-scope"><h5>"Great product and prices I am so glad I found this site. They have so many great things and so many things I want!"<span>-Robin B</span></h5></div>').insertAfter(this);
				        }
				        else{
