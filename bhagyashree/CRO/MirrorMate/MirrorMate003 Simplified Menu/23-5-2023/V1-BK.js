	...
	if (typeof jQuery != 'undefined') {
		...
		function addCustSubMenuForShopOrder(){
			...
			if(jQuery(window).width() <= 1023 ) {
				...
				console.log("LEN >> " + jQuery('#menu li.has-dropdown.has-dropdown--mega-menu.mainmenu-2').length);
				...
				console.log("LEN >> " + jQuery('header.site-header nav.nav-bar ul.dropdown.dropdown--mega-menu.menu-1').length);
	    ...
	    jQuery('.mobile-icons-left .mobile-menu a.toggle-menu').on('click', function(){
	    	jQuery(this).toggleClass("custOpen");
	    	var $this    = jQuery(this);
	    	var intCount = 0, intTotalHt = 0, intHeaderTopHt = 0, intHeaderHt = 0;
	    	
	    	

	    	if( jQuery('#qab_container').length > 0 ) {
	    		intHeaderTopHt = jQuery('#qab_container').outerHeight();
	    	}

	    	if( jQuery('#shopify-section-header').length > 0 ) {
	    		intHeaderHt = jQuery('#shopify-section-header').outerHeight();
	    	}

			intTotalHt = parseInt(intHeaderTopHt) + parseInt(intHeaderHt);

	    	var intInterval = setInterval(function () {
	    		intCount   += 1;
	    		console.log($this.hasClass("custOpen"))
				
				if($this.hasClass("custOpen") && intTotalHt > 0) {
                    intTotalHt = intTotalHt + 'px';
		    		//jQuery('body').removeClass('panel-open');
		    		console.log("intTotalHt >>> " + intTotalHt);
		    		//jQuery('html.panel-open #page nav#menu').css('top',intTotalHt+'px !important');
		    		jQuery('body.panel-open #page nav#menu').attr('style', 'top:'+ intTotalHt + ' !important');
            	    jQuery('body.panel-open #page nav#menu').offset({'top': intTotalHt});
                  
                  jQuery('html.panel-open nav#menu, body.panel-open nav#menu').addClass('custShowNav');
					//jQuery('body.panel-open #page nav#menu').offset({ top: intTotalHt });
		    		clearInterval(intInterval);
		    	}

		    	if(intCount === 80){
		    		if(!jQuery('body').hasClass("panel-open")){
		    			console.log("menu closed")
		    			jQuery('body.panel-open #page nav#menu').attr('style', 'top: 0px !important');
		    		}
					clearInterval(intInterval);
		    	}
