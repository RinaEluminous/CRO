var waitForjQuery = setInterval(function() {
    if (typeof jQuery != 'undefined' && window.MirrorMateString1DeskCtrl && window.MirrorMateString2DeskCtrl) {
        var flag = 0;

        //function to add custom HTML to the Home Page
        if (jQuery('.template-index').length > 0) {
            addCustHTMLForHpEducationFirstCtrl();
        }
        ...
        function addCustHTMLForHpEducationFirstCtrl() {
            ...
            //add custom HTML
            if (jQuery('.custMirrorMateHtmlSectionCtrl').length == 0) {
                jQuery('<div class="custMirrorMateHtmlSectionCtrl">' + window.MirrorMateString1DeskCtrl + window.MirrorMateString2DeskCtrl + '</div>').prependTo('.main-content');
            }
            ...
            jQuery('.custHowItWorksVideoPopupCtrl button#custPlayButtonCtrl').trigger('click');


            if (jQuery('.shopify-section .custom-content .content .richtext .custBlueButtonCtrl').length == 0) {
                jQuery('<a href="https://www.mirrormate.com/collections/all-frames" class="custBlueButtonCtrl">SHOP POPULAR FRAMES</a>').appendTo('.shopify-section .custom-content .content .richtext');
            }

            //re-position the FQA at the last after the default section
            if (jQuery('.main-content .shopify-section:last').length > 0) {
                jQuery('.custFAQBlockCtrl').insertAfter('.main-content .shopify-section:last');
            }
            ...
            var intSlickInterval = setInterval(function() {
                ...
                if (typeof jQuery().slick !== 'undefined') {
                    ...
                    jQuery(".custMostPopularColorSliderCtrl").on("init", function(event, slick) {
                        maxColorPages = Math.ceil(slick.slideCount / colorSlidesPerPage);
                    });

                    jQuery(".custMostPopularColorSliderCtrl").slick({
                        dots: true,
                        infinite: true,
                        arrows: true,
                ...
                if (intSlickCnt === 80) {
                    jQuery(".custMostPopularColorSliderCtrl").show();
                    clearInterval(intSlickInterval);
                }
            }, 50);
        }
        ...
        jQuery(document).on('click', '.custFAQBlockCtrl input[type="checkbox"]', function() {
            jQuery('.custFAQBlockCtrl input[type="checkbox"]').not(this).prop("checked", false);
        });
        ...
        jQuery(document).on('click', '.custHowItWorksVideoPopupCtrl button#custPlayButtonCtrl', function() {
            jQuery(this).hide();
            var mediaVideo = jQuery("#custHowItWorkVideoCtrl").get(0);
            mediaVideo.play();
        });

        //on click of the video CTA, pause the video
        jQuery(document).on('click', '.custHowItWorksVideoPopupCtrl #custHowItWorkVideoCtrl', function() {
            var mediaVideo = jQuery("#custHowItWorkVideoCtrl").get(0);
            mediaVideo.pause();
            jQuery('.custHowItWorksVideoPopupCtrl button#custPlayButtonCtrl').show();
        });

        //on click of the Works On Any Mirror image redirect to FAQ
        jQuery(document).on('click', '.custWorksOnAnyMirrorCtrl ul li a', function() {
