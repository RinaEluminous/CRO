var waitForjQuery = setInterval(function() {
    if (typeof jQuery != 'undefined' && window.MirrorMate012String1 && window.MirrorMate012String2) {
		...
		function addCustHTMLForHpEducationFirst () {
			...
			//add custom HTML
			if (jQuery('.custMirrorMateHtmlSectionHp').length == 0) {
				jQuery('<div class="custMirrorMateHtmlSectionHp">'+window.MirrorMate012String1+window.MirrorMate012String2+'</div>').prependTo('.main-content');
			}
			...
			var intInterval = setInterval(function() {
				intCount   += 1;
				console.log("Video Len >> " + jQuery('#custHowItWorkVideo').length);
				if( jQuery('#custHowItWorkVideo').length > 0 ){
					jQuery('#custHowItWorkVideo').attr({
					    "loop":"true", 
					    "autoplay":"true",
					    "muted":"",
			...
			if (jQuery('.shopify-section .custom-content .content .richtext .customBlueButtonHp').length == 0) {
				jQuery('<a href="https://www.mirrormate.com/collections/all-frames" class="customBlueButtonHp">SHOP POPULAR FRAMES</a>').appendTo('.shopify-section .custom-content .content .richtext');
			}
		
			//re-position the FQA at the last after the default section
			if(jQuery('.main-content .shopify-section:last').length > 0){
				jQuery('.custFAQBlockHp').insertAfter('.main-content .shopify-section:last');	
			}		
			...
			var intSlickInterval = setInterval(function() {
				...
				if (typeof jQuery().slick !== 'undefined') {
					...
					jQuery(".custMostPopulerColorHpSlider").on("init", function(event, slick) {
						maxColorPages = Math.ceil(slick.slideCount / colorSlidesPerPage);
					});
		
					jQuery(".custMostPopulerColorHpSlider").slick({
						dots: true,
						infinite: true,
						arrows: true,
					...
					//set the number of dots to show for slider
					jQuery(".custClientReviewSliderHp").on("init", function(event, slick) {
						maxPages = Math.ceil(slick.slideCount / slidesPerPage);
					});
		
					jQuery(".custClientReviewSliderHp").slick({
					...
					jQuery(".custMainVideoBlockHp").slick({
						slidesToShow: 3,
						infinite: true,
						slidesToScroll: 1,
						arrows: true,
				...
				if (intSlickCnt === 80) {
					jQuery(".custMostPopulerColorHpSlider").show();
					clearInterval(intSlickInterval);
				}
			}, 50);
		}
		...
		//on click of the FAQ open, close all other FQA accodion and open recent one
		 jQuery(document).on('click','.custFAQBlockHp input[type="checkbox"]',function(){
			 jQuery('.custFAQBlockHp input[type="checkbox"]').not(this).prop("checked", false);
		 });	
		...
		jQuery(document).on('click','.custHowItWorksVideoPopupHp button#custPlayButton',function(){
			 jQuery(this).hide();
		...
		//on click of the video CTA, pause the video
		 jQuery(document).on('click','.custHowItWorksVideoPopupHp #custHowItWorkVideo',function(){
			 var mediaVideo = jQuery("#custHowItWorkVideo").get(0);
			 mediaVideo.pause();
			 jQuery('.custHowItWorksVideoPopupHp button#custPlayButton').show();
		 });	
		
		 //on click of the Works On Any Mirror image redirect to FAQ
		 jQuery(document).on('click','.custWorksOnAnyMirrorHp ul li a',function(){
