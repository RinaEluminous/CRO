var waitForjQuery = setInterval(function() {
    if (typeof jQuery != 'undefined' && window.MirrorMate012String1Mob && window.MirrorMate012String2Mob) {
		var flag = 0;
	
		//function to add custom HTML to the Home Page
		if(jQuery('.template-index').length > 0){
			addCustHTMLForHpMobEducationFirst();
		}
		...
		function addCustHTMLForHpMobEducationFirst () {
			...
			//add custom HTML
			if (jQuery('.custMirrorMateHtmlSectionHpMob').length == 0) {
				jQuery('<div class="custMirrorMateHtmlSectionHpMob">'+window.MirrorMate012String1Mob+window.MirrorMate012String2Mob+'</div>').prependTo('.main-content');

				setTimeout(function (){
					jQuery('.custHowItWorksVideoPopupHpMob').addClass('custPlayVideoMob');
					jQuery('.custHowItWorksVideoPopupHpMob button#custPlayButton')[0].click();
				},3500);
			}
			...
			if (jQuery('.shopify-section .custom-content .content .richtext .customBlueButtonHpMob').length == 0) {
				jQuery('<a href="https://www.mirrormate.com/collections/all-frames" class="customBlueButtonHpMob">SHOP POPULAR FRAMES</a>').appendTo('.shopify-section .custom-content .content .richtext');
			}
		
			//re-position the FQA at the last after the default section
			if(jQuery('.main-content .shopify-section:last').length > 0){
				jQuery('.custFAQBlockHpMob').insertAfter('.main-content .shopify-section:last');	
			}		
			...
			var intSlickInterval = setInterval(function() {
				...
				if (typeof jQuery().slick !== 'undefined') {
					...
					jQuery(".custMostPopulerColorHpMobSlider").on("init", function(event, slick) {
						maxColorPages = Math.ceil(slick.slideCount / colorSlidesPerPage);
					});
		
					jQuery(".custMostPopulerColorHpMobSlider").slick({
						dots: true,
						infinite: true,
						arrows: true,
					...
					//set the number of dots to show for slider
					jQuery(".custClientReviewSliderHpMob").on("init", function(event, slick) {
						maxPages = Math.ceil(slick.slideCount / slidesPerPage);
					});
		
					jQuery(".custClientReviewSliderHpMob").slick({
					...
					jQuery(".custMainVideoBlockHpMob").slick({
						slidesToShow: 3,
						infinite: true,
						slidesToScroll: 1,
						arrows: true,
				...
				if (intSlickCnt === 80) {
					jQuery(".custMostPopulerColorHpMobSlider").show();
					clearInterval(intSlickInterval);
				}
			}, 50);
		}
		...
		//on click of the FAQ open, close all other FQA accodion and open recent one
		 jQuery(document).on('click','.custFAQBlockHpMob input[type="checkbox"]',function(){
			 jQuery('.custFAQBlockHpMob input[type="checkbox"]').not(this).prop("checked", false);
		 });	
		...
		jQuery(document).on('click','.custHowItWorksVideoPopupHpMob button#custPlayButton',function(){
			 jQuery(this).hide();
		...
		//on click of the video CTA, pause the video
		 jQuery(document).on('click','.custHowItWorksVideoPopupHpMob #custHowItWorkVideo',function(){
			 var mediaVideo = jQuery("#custHowItWorkVideo").get(0);
			 mediaVideo.pause();
			 jQuery('.custHowItWorksVideoPopupHpMob button#custPlayButton').show();
		 });	
		
		 //on click of the Works On Any Mirror image redirect to FAQ
		 jQuery(document).on('click','.custWorksOnAnyMirrorHpMob ul li a',function(){
