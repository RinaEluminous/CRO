var waitForjQuery = setInterval(function() {
    if (typeof jQuery != 'undefined' && window.MirrorMateString1MobCtrl && window.MirrorMateString2MobCtrl) {
        var flag = 0;

        //function to add custom HTML to the Home Page
        if (jQuery('.template-index').length > 0) {
            addCustHTMLForHpEducationFirstMobCtrl();
        }
        ...
        function addCustHTMLForHpEducationFirstMobCtrl() {
            ...
            //add custom HTML
            if (jQuery('.custMirrorMateHtmlSectionMobCtrl').length == 0) {
                jQuery('<div class="custMirrorMateHtmlSectionMobCtrl">' + window.MirrorMateString1MobCtrl + window.MirrorMateString2MobCtrl + '</div>').prependTo('.main-content');
            }
            ...
            jQuery('.custHowItWorksVideoPopupMobCtrl button#custPlayButtonMobCtrl').trigger('click');


            if (jQuery('.shopify-section .custom-content .content .richtext .custBlueButtonMobCtrl').length == 0) {
                jQuery('<a href="https://www.mirrormate.com/collections/all-frames" class="custBlueButtonMobCtrl">SHOP POPULAR FRAMES</a>').appendTo('.shopify-section .custom-content .content .richtext');
            }

            //re-position the FQA at the last after the default section
            if (jQuery('.main-content .shopify-section:last').length > 0) {
                jQuery('.custFAQBlockMobCtrl').insertAfter('.main-content .shopify-section:last');
            }
            ...
            var intSlickInterval = setInterval(function() {
                ...
                if (typeof jQuery().slick !== 'undefined') {
                    ...
                    var colorSlidesPerPage = 6,
                        maxColorPages = 0,
                        slidesPerPage = 5,
                        maxPages = 0;

                    //set the number of dots to show for slider
                    jQuery(".custMostPopularColorSliderMobCtrl").on("init", function(event, slick) {
                        maxColorPages = Math.ceil(slick.slideCount / colorSlidesPerPage);
                    });

                    jQuery(".custMostPopularColorSliderMobCtrl").slick({
                        dots: true,
                        infinite: true,
                        arrows: true,
                ...
                if (intSlickCnt === 80) {
                    jQuery(".custMostPopularColorSliderMobCtrl").show();
                    clearInterval(intSlickInterval);
                }
            }, 50);
        }
        ...
        jQuery(document).on('click', '.custFAQBlockMobCtrl input[type="checkbox"]', function() {
            jQuery('.custFAQBlockMobCtrl input[type="checkbox"]').not(this).prop("checked", false);
        });
        ...
        jQuery(document).on('click', '.custHowItWorksVideoPopupMobCtrl button#custPlayButtonMobCtrl', function() {
            jQuery(this).hide();
            var mediaVideo = jQuery("#custHowItWorkVideoMobCtrl").get(0);
            mediaVideo.play();
        });

        //on click of the video CTA, pause the video
        jQuery(document).on('click', '.custHowItWorksVideoPopupMobCtrl #custHowItWorkVideoMobCtrl', function() {
            var mediaVideo = jQuery("#custHowItWorkVideoMobCtrl").get(0);
            mediaVideo.pause();
            jQuery('.custHowItWorksVideoPopupMobCtrl button#custPlayButtonMobCtrl').show();
        });

        //on click of the Works On Any Mirror image redirect to FAQ
        jQuery(document).on('click', '.custWorksOnAnyMirrorMobCtrl ul li a', function() {
