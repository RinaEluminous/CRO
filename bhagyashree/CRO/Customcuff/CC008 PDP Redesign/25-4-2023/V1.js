   ...
   if (typeof jQuery != 'undefined') {
        ...
        addCustomQuantitySelector();
   		...
   		function addCustomQuantitySelector(){
   			...
   			if( jQuery('.custSelectraper').length == 0 && strSingleImg !== "" && strMatchingImg !== "" ){
   				jQuery('<div class="custSelectraper"><h3>Quantity</h3><div class="custQuanityProducts"><ul><li class="custQuanityactive custSingleCuffCTA custDisabled"><div><img src="'+strSingleImg+'"></div><span>'+strSingleTitle+'</span></li><li class="custMatchingCuffCTA custDisabled '+strClass+'"><label class="custGreenLabel">15% off</label><div><img src="'+strMatchingImg+'"></div><span>'+strMatchingTitle+'</span></li></ul></div></div>').insertBefore('#Product__AddToCart');
			}
   			...
   			var intInterval = setInterval(function () {
   				...
   				if( jQuery('.Product__Wrapper button#Product__Clone').attr('disabled') === undefined ){
	   				jQuery('.custQuanityProducts ul li').removeClass('custDisabled');
