	...
	function onInitialPageLoad(){
		...
		var interval = setInterval(function(){
			...
			if(jQuery('.js-product_section .gallery-wrap').length > 0){
	...
	function addCustomSliderToPDP(){
		...
		//replace the text of the description
		if(jQuery('.description .shg-rich-text.shg-theme-text-content span').length > 0){
			var strProdDesc = jQuery('.description .shg-rich-text.shg-theme-text-content span').text();
			strProdDesc     = strProdDesc.replace('Hemorrhoids','Nail Fungus').replace('Eczema','Neuropathy');
			jQuery('.description .shg-rich-text.shg-theme-text-content span').text(strProdDesc);
		}
