var waitForJquery = setInterval(function() {  
    ...
    if (typeof jQuery != 'undefined') {
        ...
        function renameNavigationMenu(){
            console.log('renameNavigationMenu >>> ');        
        ...
        function addCustomHTMLMobile(){
            if(jQuery('.custMobileShippingBox').length == 0){
                jQuery('<div class="custMobileShippingBox  shippingBox"><img src="https://cdn.shopify.com/s/files/1/0549/3181/6637/files/FOOTER_ICON_2-02_1_600x600.png?v=1617357333">Free Shipping Over $75</div>').prependTo('#header[class*="mobile_nav-fixed"]');

                if(jQuery('.customTopBanner').length > 0){
                    var intHeight     = jQuery('.custMobileShippingBox.shippingBox').outerHeight();
        ...
        jQuery("body").on("click", function(event) {
            if(jQuery(event.target).parents('.custSearchNavIcon').length == 1 && (event.target.id !== 'searchbox' || event.target.id === '') ){
                //jQuery('.custSearchNavIcon').attr('style', 'display: none !important');
        ...
        jQuery(document).on("click", "a.mobile_nav.dropdown_link",  function(){
            console.log('on menu open >>');
            if(isMobilFlag == 0){ 
                 console.log('call renameNavigationMenu for mobile >>');
                renameNavigationMenu();
                isMobilFlag = 1;
            }
