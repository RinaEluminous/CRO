...
function addCustomHTMLMobile(){
    ...
    if(jQuery('.custMobShippingBox').length == 0){
        jQuery('<div class="custMobShippingBox  shippingBox"><img src="https://cdn.shopify.com/s/files/1/0549/3181/6637/files/FOOTER_ICON_2-02_1_600x600.png?v=1617357333">Free Shipping Over $75</div>').prependTo('#header[class*="mobile_nav-fixed"]');

        if(jQuery('.customTopBanner').length > 0){
            var intHeight     = jQuery('.custMobShippingBox.shippingBox').outerHeight();
...
function addCustomHTMLDesktop(){
    if(jQuery('.custNavLinksMain').length == 0 && jQuery('header:not(#header) .tmenu--wrap nav.tmenu_navbar ul.tmenu_nav').length > 0) {
            jQuery('<div class="custNavLinksMain"> <div class="tmenu_item tmenu_item--root tmenu_item_level_0 tmenu_item_submenu_type_flyout tmenu_item_has_child custCallNav"> <a class="tmenu_item_link"> <span class="tmenu_item_text"> <img src="https://growth-hit.s3.us-west-2.amazonaws.com/emuaid/call_icon.svg"> </span> </a> <ul class="tmenu_submenu tmenu_submenu_type_flyout tmenu_submenu--desktop tmenu_submenu_flyout_position_automatic"> <li> <a href="tel:+18008813900"> <div class="icon"> <img src="https://growth-hit.s3.us-west-2.amazonaws.com/emuaid/call_icon.svg"> </div><div class="text"> Call Toll Free <span>1-800-881-3900</span> </div></a> </li><li> <a href="tel:+18884501718"> <div class="icon"> <img src="https://growth-hit.s3.us-west-2.amazonaws.com/emuaid/text_icon.svg"> </div><div class="text">Text Toll Free <span>1-888-450-1718</span> </div></a> </li><li class="time"> <a> <div class=" icon"> <img src="https://growth-hit.s3.us-west-2.amazonaws.com/emuaid/clock_icon.svg"> </div><div class="text">M-F: 8:30AM-6PM EST <span>Sat-Sun Closed</span></div></a> </li></ul> </div></div>').insertAfter('header.jsc-main-nav-menu .tmenu_navbar');

            //append only to desktop main navigation and not the sticky navigation
            if(jQuery('.custCartContainer').length == 0 && jQuery('.custNavLinksMain').length > 0) {
                jQuery('header.jsc-main-nav-menu .header .top-bar--right-menu .cart-container').addClass("custCartContainer");
                jQuery('header.jsc-main-nav-menu .header .top-bar--right-menu .cart-container').appendTo('.custNavLinksMain');
            }

            if(jQuery('.custSearchContainer').length == 0 && jQuery('.custNavLinksMain').length > 0) {
              jQuery('header.jsc-main-nav-menu .jsc-search-container').addClass("custSearchContainer");
              jQuery('header.jsc-main-nav-menu .jsc-search-container').insertAfter('.custNavLinksMain .custCallNav');
            }            


            if(jQuery('.custSearchNav').length == 0 && jQuery('.custCallNav').length > 0) {
                jQuery('<div class="tmenu_item tmenu_item--root tmenu_item_level_0 tmenu_item_submenu_type_flyout tmenu_item_has_child custSearchNav"> <a  class="tmenu_item_link"> <span class="tmenu_item_text"><img class="custSearchIcon" src="https://growth-hit.s3.us-west-2.amazonaws.com/emuaid/search-icon.svg"></span></a></div>').insertAfter('.custCallNav');
            }
   ...
   //hide search input(if visisble) when user click any where on page
    jQuery("body").on("click", function(event) {
        if(jQuery(event.target).parents('.custSearchNav').length == 0 && jQuery('header.jsc-main-nav-menu .header .search-container').css('width') == "100%" && event.target.id !== 'searchbox'){
           jQuery('header.jsc-main-nav-menu .header .search-container .search__form')[0].reset();
           jQuery('header.jsc-main-nav-menu .header .search-container').toggle();
        }
    });

    // On custom search icon click
    jQuery(document).on('click','.custSearchNav', function (){
        jQuery('header.jsc-main-nav-menu .header .search-container').toggle();
    });
