    ...
    function addCustQuizCtaAndPopup() {
        ...
        if(jQuery('.tmenu_item a.tmenu_item_link span.tmenu_item_text:contains(About EMUAID)').length > 0){
            ...
            jQuery('.tmenu_item a.tmenu_item_link:contains(About EMUAID)').attr('title','Why EMUAID®');

            jQuery('.tmenu_item a:contains(About EMUAID)').find('span.tmenu_item_text').text('Why EMUAID®'); 
        }
    ...
    function addCustomHTMLDesktop(){
        if(jQuery('.customQuizCta').length == 0 && jQuery('header:not(#header) .tmenu--wrap nav.tmenu_navbar ul.tmenu_nav').length > 0) {
                ...
                if(jQuery('.custShippingWrap').length == 0 && jQuery('header.jsc-main-nav-menu .header ul.social_icons + ul.menu').length > 0){
                    jQuery('<li class="custShippingWrap"><div class="shippingBox"><img src="https://cdn.shopify.com/s/files/1/0549/3181/6637/files/FOOTER_ICON_2-02_1_600x600.png?v=1617357333">Free Shipping Over $75</div></li>').appendTo('header.jsc-main-nav-menu .header ul.social_icons + ul.menu');
                }
       ...
       //hide search input(if visisble) when user click any where on page
        jQuery("body").on("click", function(event) {
            if(jQuery(event.target).parents('.custSearchNav').length == 0 && jQuery('header.jsc-main-nav-menu .header .search-container').css('display') == "block" && event.target.id !== 'searchbox'){
