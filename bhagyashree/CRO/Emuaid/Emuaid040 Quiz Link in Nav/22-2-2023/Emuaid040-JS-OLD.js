        ...
        function addCustomHTMLDesktop(){
            ...
            if(jQuery('.custSearchNav').length == 0 && jQuery('.custCallNav').length > 0) {
                jQuery('<div class="tmenu_item tmenu_item--root tmenu_item_level_0 tmenu_item_submenu_type_flyout tmenu_item_has_child custSearchNav"> <a href="javascript:;" target="_self" title="Call EMUAID®" class="tmenu_item_link"> <span class="tmenu_item_text"><img src="https://growth-hit.s3.us-west-2.amazonaws.com/emuaid/search-icon.svg" class="custSearchIcon"></span></a></div>').insertAfter('.custCallNav');
            }
        ...
        jQuery("body").on("click", function(event) {
            if(jQuery(event.target).parents('.custSearchNav').length == 0 && jQuery('header.jsc-main-nav-menu .header .search-container').css('display') == "block"){
