var waitForjQuery = setInterval(function() {
    if (typeof jQuery != 'undefined') {
			...
			if(jQuery(window).width() < 991){
				...
				if( sessionStorage.getItem("custIsBundlesClick") !==null && window.location.pathname.indexOf("all-products") !== -1){
					...
					if(orientation == 'portrait') {
						intTopOffset = jQuery('.container .image-cover').siblings('div:contains(BUNDLES)').offset().top-10;
					}
					else{
						intTopOffset = jQuery('.container .image-wrapper').siblings('div:contains(BUNDLES)').offset().top;
					}	
		...
		jQuery(document).on('click', '.custBundlesLink', function(){
			 ...
			 window.location = "https://trtltravel.com/collections/all-products";
		});
