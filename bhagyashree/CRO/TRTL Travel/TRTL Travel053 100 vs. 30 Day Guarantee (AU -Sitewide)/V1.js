var waitForjQuery = setInterval(function() {
	if (typeof jQuery != 'undefined') {
		...
		function updateNoOfMoneyBackDays() {
			...
			if(jQuery('.shopify-section .image-wrapper picture img.img-fluid[src*=100-day-money-back_YELLOW]').length > 0){
				
				jQuery('.shopify-section .image-wrapper picture img.img-fluid[src*=100-day-money-back_YELLOW]').addClass('custBlackBannerIcon');
			...
			if(jQuery('.shopify-section .image-wrapper picture img.img-fluid[src*=100-day]').not('[src*=100-day-money-back_YELLOW] , [src*=100-days_yellow_]').length > 0 ) {

				jQuery('.shopify-section .image-wrapper picture img.img-fluid[src*=100-day]').not('[src*=100-day-money-back_YELLOW] , [src*=100-days_yellow_]').addClass('customIconImg');
		...
		jQuery(document).on('click','.product-container form[id^=product-form-template-] button.add-to-cart-button', function (){
			var intCount = 0;
			var intInterval =  setInterval(function () {
				intCount += 1;
				console.log("Len >> " + jQuery('.cart-offcanvas #cart-offcanvas-content .row div:contains(100 DAY)').length);
		...
		function miniCartIconTextUpdate(){
			...
			if(jQuery('.cart-offcanvas #cart-offcanvas-content  .image-wrapper picture img.img-fluid[src*=100-day]').length > 0 ){
