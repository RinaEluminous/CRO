var waitForJquery = setInterval(function() { 
	...
	console.log("%c Travel037", "background:red;color : white;padding:10px");
    ...
    jQuery(document).on('click','.custProductsInfoshortAdding .custSwatchWrapper .custSwatchColorPattern', function() {
    ...
    //on 'size' click
    jQuery(document).on('click','.custProductsInfoshortAdding .custSizeChart label', function() {
    ...
    function checkIsAvailable() {
    	...
    	jQuery.get('https://trtltravel.com/products/trtl-compression-socks?variant='+strSelectedProdId+'&section_id=template--15670323839169__main', function(html) {
    		if(jQuery(html).find(".add-to-cart-button span").length > 0){
    			  console.log(jQuery(html).find(".add-to-cart-button span:eq(0)").text().trim());

    			var strCtaText  = jQuery(html).find(".add-to-cart-button span:eq(0)").text().trim();
    			console.log("strCTAText >> "+ strCtaText);
