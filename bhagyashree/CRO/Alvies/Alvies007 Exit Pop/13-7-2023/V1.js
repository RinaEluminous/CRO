...
var waitForJquery = setInterval(function() {  
    ...
    if (typeof jQuery != 'undefined') {
      ...
      console.log("%c Alvies007 after JS", "background:red;color : white;padding:10px");
		...
		if( document.querySelector("#checkout-main") !== null && sessionStorage.getItem('custIsOfferRedeemed') !== null ) {
			setAutoCouponCodeAtCheckout();
		}		

		//custom offer popup
		function addCustomExitPopup(){
			var intProdInCart = 0;
			if(jQuery('.utility-nav__item-count').length > 0){
				intProdInCart = jQuery('.utility-nav__item-count').text().replace(/[^\d]/g,'');
			}
			
			if( (  jQuery('.template-cart').length  > 0 && intProdInCart > 0 ) && jQuery('.customExitPopup').length == 0 && sessionStorage.getItem('custIsPopupClose') === null ) {
				
				jQuery('<div class="customExitPopup"> <div class="main"> <h2>Wait</h2> <p>You still have items in your cart</p><div class="offerMsg">Save 10% OFF Today</div><button class="redeemBtn">Redeem Now</button> <div class="offerTiming"> Special Offer Ends In: <div class="timer">01:00</div></div><button class="Search__Close"> <svg class="Icon Icon--close " role="presentation" viewBox="0 0 16 14"> <path d="M15 0L1 14m14 0L1 0" stroke="currentColor" fill="none" fill-rule="evenodd"></path> </svg> </button> </div><div class="overlay"></div></div>').appendTo('body');
		...
		function setAutoCouponCodeAtCheckout(){
		...
		jQuery(document).on('click','.customExitPopup .main button.Search__Close,.customExitPopup .overlay', function(){
			jQuery('.customExitPopup').fadeOut();
			sessionStorage.setItem('custIsPopupClose','1');
		});

		//on redeem CTA click
		jQuery(document).on('click','.customExitPopup .main button.redeemBtn', function(){
				jQuery('.customExitPopup').fadeOut();
				sessionStorage.setItem('custIsPopupClose','1');
				sessionStorage.setItem('custIsOfferRedeemed','1');
		...
		function offerCountDownTimer() {
			var timer2 = intCountDownTime;
				...
				jQuery('.customExitPopup .main .offerTiming .timer').html(countDownTime);
