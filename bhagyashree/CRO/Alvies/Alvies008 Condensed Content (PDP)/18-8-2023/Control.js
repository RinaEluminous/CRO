var waitForJquery = setInterval(function() { 
    if (typeof jQuery != 'undefined') {
        ...
        addCustHTMLToPdpControl();

        function addCustHTMLToPdpControl(){
            var intCount    = 0 ;
            ...
            if(jQuery('.custCheckListControl').length  == 0 ) {
                jQuery('<div class="custCheckListControl"><ul><li><img src="https://i.ibb.co/NnjwPQ4/np-check.png">Lifetime Guarantee</li><li><img src="https://i.ibb.co/NnjwPQ4/np-check.png">Free shipping & returns</li></ul></div>').insertBefore('.product-form');
            }

            if(jQuery('.custTestimonialsBlueWrapperControl').length == 0){
                ...
                if((jQuery('.product-info .badges').length > 0 ) && (strProdCategory == "The Trinity" || strProdCategory == "The Stassney") ){
                	jQuery('.product-info .badges').addClass('custProductBadgesControl');
                }

                if(strReviews !== undefined && strReviews !=='' && strReviews !== null ) {
                    jQuery('<div class="custTestimonialsBlueWrapperControl"><h6>What Customers Say</h6><p>"'+strReviews[0]+'"</p><span>'+strReviews[1]+'</span></div>').insertAfter('.product-form__form .product-form__add');
                }
        ...
        function addReviewsControl(){
            if( jQuery('.product-info__review-summary .oke-sr-count').length > 0  && jQuery('.custPdpReviewsControl').length == 0){
                ...
                if(jQuery('div.okeReviews .oke-w-reviews-writeReview a.oke-w-writeReview').length > 0){
                    custHTML += ' <button class="custWriteReviewBtnControl"> Write a review</button>';
                }
                
                custHTML = custHTML.replace(/\ Reviews\b/g, '');
                jQuery('<div class="custPdpReviewsControl">'+custHTML+'</div>').insertAfter('.product-info__review-summary .oke-sr .oke-is-clickable');
            }
        }

        //on click - Write a review CTA
        jQuery(document).on('click','.custWriteReviewBtnControl', function (){
