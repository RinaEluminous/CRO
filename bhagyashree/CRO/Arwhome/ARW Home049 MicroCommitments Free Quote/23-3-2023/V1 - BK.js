var waitForJquery = setInterval(function() { 
    if (typeof jQuery != 'undefined') {
        ...
        jQuery(document).on('click','.container.custQuoteMainSection .custQuoteStartSection#application button.btn.custQuoteBtn', function(){
            ...
            console.log("Home field Len >> " + jQuery('input[name="what_type_of_home_do_you_have_"]').length);
            ...
            var strSystemApplications = jQuery(".container.custQuoteMainSection .custQuoteStartSection#application ul li .custIconBox.active").map(function() {
             ...
             console.log("strHomeType  >> " + strHomeType);
             console.log("strSystemApplications  >> " + strSystemApplications);


            /*if(jQuery('input[name="what_type_of_home_do_you_have_"]').length > 0 ){
                console.log("set home value  >> " );
                    jQuery('input[name="what_type_of_home_do_you_have_"]').val(strHomeType);
                }

            console.log("Appliances field Len >> " + jQuery('input[name="what_systems_and_appliances_are_you_wanting_to_protect_"]').length);
            if(jQuery('input[name="what_systems_and_appliances_are_you_wanting_to_protect_"]').length > 0 ){

                console.log("set Appliances value  >> " );
                jQuery('input[name="what_systems_and_appliances_are_you_wanting_to_protect_"]').val(strSystemApplications);
            }*/



            // get the iframe element
            var iframe = jQuery('#hs-form-iframe-1'); // get the form input element inside the iframe
            var inputHomeType = iframe.contents().find('input[name="what_type_of_home_do_you_have_"]'); // set the value of the input element
            inputHomeType.val(strHomeType);

            var inputSystemApplications = iframe.contents().find('input[name="what_systems_and_appliances_are_you_wanting_to_protect_"]'); // set the value of the input element
            inputSystemApplications.val(strSystemApplications);

            jQuery('iframe#hs-form-iframe-1').css('display','block !important');
            //jQuery('iframe#hs-form-iframe-1').attr('style', 'display:block !important');
